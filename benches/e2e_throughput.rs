...
//! cargo bench --bench e2e_throughput -- --samples simple_fib,pattern_matching
//!
//! # Run multiple samples
//! cargo bench --bench e2e_throughput -- --duration 60 --samples simple_fib,pattern_matching,metta_programming_stress
//!
//! # Run samples for extended period
//! cargo bench --bench e2e_throughput -- --duration 300 --samples simple_fib,knowledge_graph
//!
//! # Show help
...
const SAMPLES: &[(&str, &str)] = &[
    ("simple_fib", include_str!("metta_samples/fib.metta")),
    (
        "knowledge_graph",
    ...
    (
        ...
        include_str!("metta_samples/contstraint_search_simple.metta"),
    ),
    (
...
fn print_scaling_analysis(reports: &[ThroughputReport]) {
    println!("\n=== Scaling Analysis ===");

    // Get unique sample names from reports
    let mut sample_names: Vec<_> = reports.iter().map(|r| r.sample_name.as_str()).collect();
    ...
    for name in sample_names {
        let seq = reports
            .iter()
            .find(|r| r.sample_name == name && r.mode == "sequential");

        if let Some(seq_report) = seq {
            println!("\n{}:", name);
            println!(
                "  sequential:    {:>10.2} prog/s (baseline)",
                seq_report.programs_per_second
            );

            for report in reports
                .iter()
                .filter(|r| r.sample_name == name && r.mode != "sequential")
            {
                let speedup = report.programs_per_second / seq_report.programs_per_second;
                let efficiency =
                    if report.mode.starts_with("parallel-") || report.mode.starts_with("async-") {
                        let workers: f64 = report
                            .mode
                            .split('-')
                            .last()
                            .and_then(|s| s.parse().ok())
                            .unwrap_or(1.0);
                        speedup / workers * 100.0
                    } else {
                        100.0
                    };

                println!(
                    "  {:14} {:>10.2} prog/s ({:.2}x speedup, {:.1}% efficiency)",
                    format!("{}:", report.mode),
                    report.programs_per_second,
                    speedup,
                    efficiency
                );
            }
...
fn main() {
    ...
    print_scaling_analysis(&reports);

    let total_programs: u64 = reports.iter().map(|r| r.total_programs).sum();
    println!("\n=== Summary ===");
    println!("Total programs evaluated: {}", total_programs);
}
