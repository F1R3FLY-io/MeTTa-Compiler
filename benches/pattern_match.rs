...
fn bench_variable_count_scaling(c: &mut Criterion) {
    ...
    for &count in &[1, 5, 10, 25, 50] {
        let pattern_items: Vec<_> = (0..count)
            .map(|i| var(&format!("v{}", i)))
            .collect();
        let value_items: Vec<_> = (0..count)
            .map(|i| MettaValue::Long(i as i64))
            .collect();
        ...
        group.bench_with_input(
            ...
            |b, (p, v)| {
                b.iter(|| {
                    black_box(pattern_match(black_box(p), black_box(v)))
                })
            },
        );
    }
...
fn bench_existing_binding_complex(c: &mut Criterion) {
    let complex_value = sexpr(vec![
        atom("a"),
        sexpr(vec![atom("b"), atom("c")]),
    ]);
...
fn bench_ground_types(c: &mut Criterion) {
    ...
    // Float
    let float_pattern = MettaValue::Float(3.14);
    let float_value = MettaValue::Float(3.14);
...
fn bench_failures(c: &mut Criterion) {
    ...
    let value2 = sexpr(vec![MettaValue::Long(1), MettaValue::Long(2), MettaValue::Long(3)]);
