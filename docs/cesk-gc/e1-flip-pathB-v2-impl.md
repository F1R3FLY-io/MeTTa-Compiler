...
Mechanical proof hook: `formal/lean/gc/RendezvousWitness.lean` and `formal/rocq/gc/RendezvousWitness.v` prove the
root-union obligation used here. If the live-re-walk wait has published every occupied slot, publication buffered
that slot's structural roots, and the driver drains the buffer into its root set, then every occupied participant
root is in the driver root set. Combined with mark-from-driver-roots and sweep-only-unmarked, those roots cannot be
freed.
