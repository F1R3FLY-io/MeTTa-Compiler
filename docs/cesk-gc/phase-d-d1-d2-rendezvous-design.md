...
## ⛔ RED-TEAM ROUND 1 VERDICT (2026-06-02): the proposed fix does NOT converge — the rendezvous-park model is fundamentally incompatible with this codebase's parallel dispatch. DO NOT implement D2.3 / D5. RECONSIDER THE APPROACH.

Four independent adversarial Plan agents (liveness / UAF-HB / leave-active-enumeration / model-faithfulness) attacked the proposed fix (in_dispatch guard + restore active==0 gate + reorder park + between-directive fallback). Round 1 was heavily NET-ADDITIVE (≥4 serious new flaws IN THE FIX) ⇒ NOT converged. The committed D2.1 (`8cfb7a4`) + uncommitted D2.3 + the proposed fix all fail:

1. **Liveness STARVATION (the fix itself starves).** `worker_ever_spawned()` is a sticky process-lifetime latch (`gc_allocator.rs:2790`, never reset) ⇒ `gate_open()`/`gate_open_midloop()` (the ST collectors AND the proposed "between-directive `run_collection_if_triggered` fallback", `eval/mod.rs:308`) are PERMANENTLY OFF under FANOUT>0. So the rendezvous is the SOLE collector. The `in_dispatch` guard then empties the requestor-eligible set for the whole directive (under nested PLN every thread is in-dispatch) ⇒ NOTHING collects under FANOUT>0 ⇒ heap grows unbounded — worse than pre-D1.
2. **Finishing-worker UAF (active==0 gate).** A finished worker's result lives in `ParallelDispatchHandle.results[slot]`, rooted ONLY via a live `WaitForParallel` continuation (`types.rs:2332`). Under index-gc the `ParallelDispatchRootProvider` is NOT registered (`eval_loop.rs:2592`, slab-only — the A5 deletion). The rendezvous union (buffer ∪ E₀ ∪ driver-C) MISSES in-flight dispatch results ⇒ swept ⇒ UAF on merge.
3. **Mid-spawn admission UAF.** `EvalGuard::enter` (`gc_allocator.rs:3460`) joins `active` WITHOUT checking `GC_REQUESTED` (only `GC_IN_PROGRESS`), and the WorkerEnter-gate→`EvalGuard::enter` pair (`eval_loop.rs:2469`→`:2473`) is non-atomic w.r.t. `request_gc()` ⇒ a mid-spawn worker joins `active` AFTER the `n` snapshot and PAST its only gate ⇒ its roots unmarked ⇒ UAF. (Risk-R2 "note_worker_spawned latches before spawn" is a non-sequitur — the latch doesn't put the worker in `active`.)
4. **Requestor-is-parent deadlock.** The requestor IS the dispatch-parent (same trampoline thread pumps `WaitForParallel` via the re-pushed continuation `eval_loop.rs:15172` AND hits the `gc_counter&0xFFF` safepoint `:3631`). So `active==0` is unreachable while it pumps (deadlock) OR it drops its own guard (re-opening #2). The SHIPPED parked-count gate (`requestor_wait_for_parked_count(n)`, `n=active-1`) is the ORIGINAL bug: unreachable when any counted worker FINISHES/cancels/panics/blocks without parking (the warn-retry loops forever).
5. **The models certify-around AGAIN** unless re-derived with: split-park (leave-active vs publish-roots as DISTINCT actions with the interleaving window), a finish-without-park transition, a dispatch-PARENT thread (active-holding + requestor-eligible), and a mid-spawn worker. The current loom (`gc_allocator.rs:7980`, unconditional-park, gates on the publish-signal itself) + TLA+ (`StoreCentricGC.tla`, single global `psi`, atomic park, symmetric peers, no dispatch nesting) structurally cannot express the bug class.

**The real requirements any parallel-collection design MUST satisfy (the red-team's convergent residue):** (a) root in-flight `handle.results` under index-gc for the WHOLE dispatch lifetime (an index-mode equivalent of `ParallelDispatchRootProvider`, OR the collector walks live dispatch handles); (b) work when the requestor is a dispatch-parent that must keep pumping; (c) handle finish-without-park + mid-spawn + cancel/panic-unwind leave-active paths; (d) NOT depend on the `worker_ever_spawned`-latched ST gates for the FANOUT>0 fallback. A hinted direction: collect at the EXISTING `pump_parallel_wait` tick (where `request_gc()` already fires for slab, `eval_loop.rs:2748`) and root dispatch handles directly, rather than parking workers at safepoints.

**Status: D2.1 stays committed but DORMANT (production byte-identical/correct). D2.3 NOT committed. D5 BLOCKED. The parallel-collection APPROACH is under reconsideration (the red-team rejected the park-at-safepoints rendezvous).**
