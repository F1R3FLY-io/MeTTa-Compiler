...
- **E5** — the three push arms (~:1287, :1321, :1364): replace `self.free_list.push(a)` with `if seg.set_free_bit(off) { self.free_list.push(a); }`. **`reclaimed_out.push(a)` stays UNGUARDED** (outside the `if`) so every swept slot is visible to the side-payload layer, but the side layer must snapshot `(segment, column, side-index)` at reclaim time and drain those snapshots at quiescence. Do not reread mutable node bytes at drain time; node-slot reuse can overwrite them before quiescence.
...
- **NO change** to `promote_young`, the sweep signatures, or `SweepStats`. The post-ASAN side-payload repair is local to `IndexHeap`: pending side snapshots replace the old `last_reclaimed` reread drain, and pending snapshots for whole-released segments are discarded because the segment reset already freed those columns.

## Detector interaction (retired after formalization)
The implementation originally kept an env-gated `on_free_list` `AtomicBool` shadow as a temporary R-FL discriminator.
After the `free_bit` lifecycle moved into the mandatory Rocq/TLA/source-coupling gate, that production environment
switch was removed. The permanent cross-check is now the source-coupled `free_bit(addr) set <=> addr is on free_list`
invariant plus the `FreeList.v`, `IndexAllocatorRefinement.v`, and formal harness checks.

## Verification (conformance does 0 GC cycles — must FORCE GC)

### Completed scoped checks

- Historical detector-backed run: `cargo test --lib --features index-gc index_arena::tests -- --nocapture` passed,
  23/23, including:
...
- Lean/Rocq theorem: `formal/lean/gc/FreeList.lean` and `formal/rocq/gc/FreeList.v` prove the unbounded free-list lifecycle (`push`, `pop`, `majorDrain`, and release-drain) preserves `free_bit(addr) set <=> addr is on free_list` and `free_list.Nodup`. Checked by `scripts/verify_cesk_gc_formal.sh`.
...
A bounded Rust harness now covers the post-fix FANOUT=0 evaluator-level non-vacuity check without relying on the earlier recursive MeTTa fixture. It builds+discards enough transient expressions to force both minor and major collection in every run; the post-fix ×20 arm is green. The pre-fix bite is discharged by `scripts/rfl_pre_fix_bite.sh`; because the detector was introduced with the fix, the durable historical check injects the no-duplicate assertion into the old checkout and requires that assertion to fail for the expected duplicate-listing reason.
