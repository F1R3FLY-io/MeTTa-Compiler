...
- Lean theorem: `formal/lean/gc/FreeList.lean` proves the unbounded free-list lifecycle (`push`, `pop`, `majorDrain`, and release-drain) preserves `free_bit(addr) set <=> addr is on free_list` and `free_list.Nodup`. Checked with `lean formal/lean/gc/FreeList.lean`.
