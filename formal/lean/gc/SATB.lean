...
realize in source: every value live in the snapshot-at-the-beginning set is
reachable from either the initial roots, a SATB-shaded deletion pre-image, or an
allocate-black root before sweep.
-/
...
theorem deleted_preimage_is_satb_root
    {InitialRoot ShadedDeletion AllocateBlack : Addr -> Prop}
    {a : Addr} :
    ShadedDeletion a -> SATBRoot InitialRoot ShadedDeletion AllocateBlack a := by
  intro h
...
theorem allocate_black_is_satb_root
    {InitialRoot ShadedDeletion AllocateBlack : Addr -> Prop}
    {a : Addr} :
    AllocateBlack a -> SATBRoot InitialRoot ShadedDeletion AllocateBlack a := by
  intro h
...
theorem no_snapshot_live_uaf
    ...
    (markComplete :
      forall {a : Addr}, Reach (SATBRoot InitialRoot ShadedDeletion AllocateBlack) Edge a -> Marked a)
    (sweepOnlyUnmarked : forall {a : Addr}, Freed a -> Not (Marked a))
    (snapshotLiveCovered :
      forall {a : Addr}, SnapshotLive a -> Reach (SATBRoot InitialRoot ShadedDeletion AllocateBlack) Edge a) :
