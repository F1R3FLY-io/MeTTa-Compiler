...
run_one() { # $1=label $2=young_min $3=logprefix
  ...
  for i in $(seq 1 "$REPLICATES"); do
    /usr/bin/env METTATRON_PARALLEL_FANOUT_DEPTH=0 \
      METTATRON_INDEX_GC_MIN_BYTES=131072 METTATRON_INDEX_GC_YOUNG_MIN_BYTES="$ymin" \
      METTATRON_INDEX_GC_REPORT=2 \
      /usr/bin/time -v $PIN "$MTT" "$WORKLOAD" >"${pfx}.out" 2>"${pfx}.err"
    local wall rss minor major
    wall=$(grep -oE "wall clock.*" "${pfx}.err" | grep -oE "[0-9:.]+$")
    rss=$(grep -oE "Maximum resident set size.*: [0-9]+" "${pfx}.err" | grep -oE "[0-9]+$")
    minor=$(grep -c "minor cycle" "${pfx}.err")
    major=$(grep -c "major cycle" "${pfx}.err")
    echo "  run $i: wall=$wall  peakRSS_kb=$rss  minors=$minor  majors=$major  result=$(tail -1 "${pfx}.out")"
  done
}

echo "===== C1.b FANOUT=0 BENCHMARK ($WORKLOAD, ${REPLICATES} replicates) ====="; date
echo "binary: $MTT"
run_one "BASELINE (minors OFF = C1.a all-majors)" 2000000000 /tmp/c1b_bench_base
run_one "MINORS (generational young sweep)"      131072     /tmp/c1b_bench_minor
