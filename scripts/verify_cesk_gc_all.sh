...
#     DETERM     20-run Robot FANOUT=8 sorted-content hash (1 distinct,
#                order-insensitive — the unsorted ORDER is pre-existing
#                nondeterministic at HEAD; content must be invariant)
#
# Resource discipline: every cargo/TLC/Rocq stage is internally capped by the
# stage scripts (systemd-run MemoryMax); the loom stage is capped here. Run on a
# machine clear of sibling cargo builds (`ps`-check first) — a loaded box can
# flake the nextest work-pool spawn (see memory: once-flake-environmental).
set -uo pipefail
LABEL="${1:-allwall}"
...
run_mmverify_both() {
  ...
  out_index="$("$LOG_DIR/mtt-index" examples/mmverify/demo0/verify_demo0.metta 2>&1 | grep -c 'Correct proof!')"
  ...
  out_slab="$("$LOG_DIR/mtt-slab" examples/mmverify/demo0/verify_demo0.metta 2>&1 | grep -c 'Correct proof!')"
  [[ "$out_slab" -ge 1 ]] || { echo "slab mmverify did not print 'Correct proof!'"; exit 1; }
  echo "mmverify Correct on both arms"
}

# ── 20-run sorted-content determinism (Robot, FANOUT=8, the INDEX binary) ────
run_determinism() {
  set -e
  [[ -x "$LOG_DIR/mtt-index" ]] || { echo "index binary missing (MMVERIFY stage builds it)"; exit 1; }
  local hashes
  hashes="$(for i in $(seq 1 20); do
    env METTATRON_PARALLEL_FANOUT_DEPTH=8 "$LOG_DIR/mtt-index" "$PLN/examples/Robot.metta" 2>/dev/null | sort | sha256sum | cut -d' ' -f1
  done | sort -u | wc -l)"
  echo "distinct sorted-content hashes: $hashes (expect 1)"
  [[ "$hashes" == "1" ]]
}
...
stage DETERM     "20-run sorted-content determinism" run_determinism                              || exit 1
