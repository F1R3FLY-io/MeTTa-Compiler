...
assert_count "src" "enter_satb_marking()" "1"
assert_zero "src/backend/eval/cesk/index_heap.rs" "mark_concurrent"
...
assert_after_before "src/backend/eval/cesk/index_heap.rs" "fn mark_sweep_if_over_watermark" "heap.mark_young(&addrs); // YOUNG-ONLY mark" "(heap.sweep_young(), false)"
...
assert_after_before "src/backend/bytecode/tiered_cache.rs" "impl Drop for PendingBytecodeRootGuard" "with_satb_deletion_barrier" "self.roots.remove(&self.expr_hash)"
assert_after_before "src/backend/bytecode/tiered_cache.rs" "impl Drop for PendingBytecodeRootGuard" "self.roots.remove(&self.expr_hash)" "shade_tiered_roots(vec![root]);"
assert_after_before "src/backend/bytecode/tiered_cache.rs" "fn register_pending_bytecode_root" "with_satb_deletion_barrier" "pending_bytecode_roots.insert(expr_hash, expr);"
assert_after_before "src/backend/bytecode/tiered_cache.rs" "fn register_pending_bytecode_root" "pending_bytecode_roots.insert(expr_hash, expr);" "shade_tiered_roots(vec![root]);"
assert_after_before "src/backend/bytecode/tiered_cache.rs" "if !enqueued {" "with_satb_deletion_barrier" ".remove(&state.expr_hash)"
assert_after_before "src/backend/bytecode/tiered_cache.rs" "if !enqueued {" ".remove(&state.expr_hash)" "shade_tiered_roots(vec![root]);"
