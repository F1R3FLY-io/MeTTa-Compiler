...
use super::jit::{JitCompiler, JitProfile};
...
/// # Type Parameters
///
/// - `V`: The value type for constants (e.g., `MettaValue` or `MettaValue`)
#[derive(Debug, Clone)]
    ...
    V: MettaValueTrait + Clone + Send + Sync + 'static,
{
    ...
    /// Create a chunk from raw bytecode and constant count (no actual constants).
    ///
    /// This is used for JIT compilation of generic bytecode chunks where the bytecode
    /// is identical but the constants are typed differently. The JIT compiler only needs
    /// the bytecode structure and constant count for code generation; actual constants
    /// are accessed via JitContext at runtime.
...
/// Disassembly methods (require V: Debug for constant formatting)
impl<V> GenericBytecodeChunk<V>
where
    V: MettaValueTrait + Clone + Send + Sync + std::fmt::Debug + 'static,
{
...
/// MettaValue-specific methods (JIT compilation requires concrete BytecodeChunk type)
impl GenericBytecodeChunk<MettaValue> {
    ...
    pub fn builder(name: impl Into<String>) -> GenericChunkBuilder<MettaValue, GcFactory> {
        GenericChunkBuilder::new(name)
    }
}
...
pub struct CompiledPattern {
    ...
    pub guard: Option<Arc<GenericBytecodeChunk<MettaValue>>>,
    ...
    F: MettaValueFactory<V>,
{
    ...
    is_vararg: bool,
    current_line: u32,
    /// Whether to enable peephole optimization (default: false)
    optimize: bool,
...
/// Convenience constructors for builders with Default factories (e.g. GcFactory).
impl<V, F> GenericChunkBuilder<V, F>
where
    V: MettaValueTrait + Clone + Send + Sync + PartialEq + 'static,
    F: MettaValueFactory<V> + Default,
{
    /// Create a new chunk builder using the default factory.
    ///
    ...
    pub fn new(name: impl Into<String>) -> Self {
        Self::with_factory(name, F::default())
    }

    /// Create a new chunk builder with optimization enabled, using the default factory.
...
where
    ...
    F: MettaValueFactory<V>,
{
    /// Create a new chunk builder with an explicit factory.
    pub fn with_factory(name: impl Into<String>, factory: F) -> Self {
        Self {
    ...
    ///
    /// When enabled, the bytecode will be optimized during build()
...
// ============================================================================
// Type Aliases (MettaValue + GcFactory specializations)
// ============================================================================

/// A compiled bytecode chunk with MettaValue constants.
pub type BytecodeChunk = GenericBytecodeChunk<MettaValue>;

/// Builder for constructing BytecodeChunks with MettaValue constants.
pub type ChunkBuilder = GenericChunkBuilder<MettaValue, GcFactory>;
...
mod tests {
    ...
    fn test_can_jit_compile_arithmetic_chunk() {
        ...
        assert!(
            chunk.can_jit_compile(),
            "Arithmetic chunk should be JIT compilable"
        );
    }
}
