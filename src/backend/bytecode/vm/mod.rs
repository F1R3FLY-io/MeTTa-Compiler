    ...
    F: MettaValueFactory<V> + Copy + Clone + Send + Sync + 'static,
{
    ...
    fn with_vm_roots_frame(&self) -> Option<crate::backend::eval::cesk::k_spine::VmLeafGuard> {
        ...
        // SAFETY: `V == MettaValue` verified; `F == ActiveFactory` at every live
        // call site; `self` outlives the guard (sibling stack local). The VM is
        // decoded structurally by `collect_k_spine` via `collect_roots_into`.
    ...
    pub fn run(&mut self) -> VmResult<Vec<V>> {
        ...
        loop {
            ...
            if iter_counter & 0xFF == 0 {
                // E1-c step 4 (design §Part-6): the poll gate fires for ANY
                // EvalGuard-holding thread that observes `is_gc_requested()` — the
                // dedicated GC thread's `requestor_wait_for_parked_count(n)` counts
                // every such thread (driver + batch + dispatch), not only
                // `IS_PARALLEL_WORKER`-flagged ones. Dedicated OFF: nothing sets
                // GC_REQUESTED under FANOUT>0, so the body never runs (and
                // `worker_cooperative_safepoint` fast-returns regardless).
