...
proptest! {
    /// Reflexivity: any value matches itself
    #[test]
    fn prop_pattern_match_reflexive(value in arb_metta_value(2)) {
        prop_assert!(pattern_matches(&value, &value));
    }

    /// Variable matches anything
    #[test]
    fn prop_variable_matches_anything(value in arb_metta_value(2)) {
        let var = MettaValue::var("x");
        prop_assert!(pattern_matches(&var, &value));
    }

    /// Wildcard matches anything
    #[test]
    fn prop_wildcard_matches_anything(value in arb_metta_value(2)) {
        let wildcard = MettaValue::sym("_");
        prop_assert!(pattern_matches(&wildcard, &value));
    }

    /// pattern_match_bind returns bindings when pattern matches
    #[test]
    fn prop_pattern_match_bind_returns_bindings(value in arb_simple_value()) {
        let var = MettaValue::var("x");
        let result = pattern_match_bind(&var, &value);

        prop_assert!(result.is_some());
        let bindings = result.unwrap();
        prop_assert!(!bindings.is_empty());
        // The binding should contain the matched value
        prop_assert!(bindings.iter().any(|(_, v)| v == &value));
    }

    /// Unification is symmetric (for simple values without variables)
    #[test]
    fn prop_unify_symmetric(a in arb_simple_value(), b in arb_simple_value()) {
        let unify_ab = unify(&a, &b);
        let unify_ba = unify(&b, &a);

        // Both should succeed or both should fail
        prop_assert_eq!(unify_ab.is_some(), unify_ba.is_some());
    }

    /// Unification with variable always succeeds
    #[test]
    fn prop_unify_with_variable(value in arb_simple_value()) {
        let var = MettaValue::var("x");

        let unify_vv = unify(&var, &value);
        prop_assert!(unify_vv.is_some());

        let unify_vv = unify(&value, &var);
        prop_assert!(unify_vv.is_some());
    }

    /// Wildcard does NOT create bindings (unlike variables)
    #[test]
    fn prop_wildcard_no_binding(value in arb_simple_value()) {
        let wildcard = MettaValue::sym("_");
        let result = pattern_match_bind(&wildcard, &value);

        // Wildcard matches but creates no bindings
        prop_assert!(result.is_some());
        let bindings = result.unwrap();
        // Wildcard should NOT add a binding for "_"
        prop_assert!(!bindings.iter().any(|(name, _)| name == "_"),
            "Wildcard should not create a binding");
    }

    /// Unification fails for different ground values
    #[test]
    fn prop_ground_inequality(a in 0i64..100i64, b in 100i64..200i64) {
        // a and b are guaranteed different by their ranges
        let val_a = MettaValue::Long(a);
        let val_b = MettaValue::Long(b);

        let result = unify(&val_a, &val_b);
        prop_assert!(result.is_none(), "Different ground values should not unify");
    }

    /// Unification fails for S-expressions with different arity
    #[test]
    fn prop_unify_arity_mismatch(
        short in prop::collection::vec(arb_simple_value(), 1..3),
        extra in arb_simple_value()
    ) {
        let short_sexpr = MettaValue::SExpr(short.clone());
        let mut long_items = short.clone();
        long_items.push(extra);
        let long_sexpr = MettaValue::SExpr(long_items);

        let result = unify(&short_sexpr, &long_sexpr);
        prop_assert!(result.is_none(), "Different arity S-expressions should not unify");
    }

    /// Binding collection: all variables in pattern appear in bindings
    #[test]
    fn prop_binding_collection(val1 in arb_simple_value(), val2 in arb_simple_value()) {
        // Pattern: ($x $y)
        let pattern = MettaValue::SExpr(vec![MettaValue::var("x"), MettaValue::var("y")]);
        // Value: (val1 val2)
        let value = MettaValue::SExpr(vec![val1.clone(), val2.clone()]);

        let result = pattern_match_bind(&pattern, &value);
        prop_assert!(result.is_some());

        let bindings = result.unwrap();
        // Should have bindings for both $x and $y (variables are stored with $ prefix)
        let has_x = bindings.iter().any(|(name, v)| name == "$x" && v == &val1);
        let has_y = bindings.iter().any(|(name, v)| name == "$y" && v == &val2);

        prop_assert!(has_x, "Should have binding for $x");
        prop_assert!(has_y, "Should have binding for $y");
    }

    /// Recursive binding: nested variables are all collected
    #[test]
    fn prop_recursive_binding(val in arb_simple_value()) {
        // Pattern: (outer $inner)
        let pattern = MettaValue::SExpr(vec![
            MettaValue::sym("outer"),
            MettaValue::var("inner"),
        ]);
        // Value: (outer val)
        ...
        let bindings = result.unwrap();
        // Variables are stored with $ prefix
        let has_inner = bindings.iter().any(|(name, v)| name == "$inner" && v == &val);
        prop_assert!(has_inner, "Should have binding for $inner");
    }
}
