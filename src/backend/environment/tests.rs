...
fn stress_concurrent_clone_and_mutate() {
    ...
    let env = StdArc::new(HeapEnvironment::default());
    let num_threads = 8;
...
fn integration_parallel_eval_with_dynamic_rules() {
    ...
    let base_env = HeapEnvironment::default();
...
mod thread_safety {
    ...
    fn test_concurrent_clone_and_mutate_2_threads() {
        let mut base = HeapEnvironment::default();
        ...
        let results: Vec<HeapEnvironment> = handles.into_iter().map(|h| h.join().unwrap()).collect();
    ...
    fn test_concurrent_clone_and_mutate_8_threads() {
        ...
        // Collect results
        let results: Vec<(usize, HeapEnvironment)> =
    ...
    fn test_concurrent_add_rules() {
        ...
        // Collect all clones
        let clones: Vec<HeapEnvironment> = handles.into_iter().map(|h| h.join().unwrap()).collect();
