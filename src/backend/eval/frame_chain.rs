...
// A5.2: `FrameAndKSpineGuard` + `maybe_push_frame` REMOVED — the module-import /
// assertion call sites now use `super::expr_vec_frame::push_expr_vec_frame` (index
// build -> typed K-spine `ExprVec`; slab build -> `EvalFrameGuard::push_vec`). The
// K-spine half was index-gc-only (runtime-gated off in production slab), so this is
// byte-identical for the slab build.
