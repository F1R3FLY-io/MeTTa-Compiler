                        ...
                        if results1.is_empty() {
                            ...
                            if let MettaValue::Space(ref handle) = first_val {
                                ...
                                if is_boolean_check {
                                    ...
                                    let matches = if handle.is_module_space() || handle.name == "self"
                                    {
                                    ...
                                    let mut bodies_to_eval = VecDeque::new();
                                    ...
                                    for matched_atom in &matches {
                                        if let Some(bindings) = pattern_match(&pattern, matched_atom)
                                        {
                                            ...
                                            pattern_match(matched_atom, &pattern)
                                        {
                                            found_match = true;
                                            let instantiated =
                                                apply_bindings(&success_body, &bindings).into_owned();
                            ...
                            if let MettaValue::Space(ref handle) = next_val {
                                ...
                                for matched_atom in &matches {
                                    if let Some(bindings) = pattern_match(&pattern, matched_atom) {
                                        ...
                                        let instantiated =
                                            apply_bindings(&success_body, &bindings).into_owned();
                                        new_bodies.push_back(instantiated);
                                    } else if let Some(bindings) =
                                        pattern_match(matched_atom, &pattern)
                                    {
                                        found_match = true;
