...
//!
//! Provides tail-call optimized arithmetic operations that work with any
...
impl<V: MettaValueTrait + Clone> GroundedOperationTCO<V> for AddOp {
    ...
    fn execute_step<F: MettaValueFactory<V>>(
        ...
        match state.step {
            ...
            _ => unreachable!("Invalid step {} for AddOp", state.step),
        }
    }
}

/// TCO Subtraction/Negation operation: (- a b) or (- a)
pub struct SubOp;

impl<V: MettaValueTrait + Clone> GroundedOperationTCO<V> for SubOp {
    fn name(&self) -> &str {
        "-"
    }
        ...
        match state.step {
            0 => {
                match state.args.len() {
                    ...
                    _ => GroundedWork::Error(ExecError::IncorrectArgument(format!(
            ...
            _ => unreachable!("Invalid step {} for SubOp", state.step),
        }
    }
}
...
impl<V: MettaValueTrait + Clone> GroundedOperationTCO<V> for MulOp {
    fn name(&self) -> &str {
        "*"
    }
    ...
    fn execute_step<F: MettaValueFactory<V>>(
        ...
        match state.step {
            ...
            _ => unreachable!("Invalid step {} for MulOp", state.step),
        }
    }
}
...
impl<V: MettaValueTrait + Clone> GroundedOperationTCO<V> for DivOp {
    fn name(&self) -> &str {
        "/"
    }
    ...
    fn execute_step<F: MettaValueFactory<V>>(
        ...
        match state.step {
            ...
            _ => unreachable!("Invalid step {} for DivOp", state.step),
        }
    }
}
...
impl<V: MettaValueTrait + Clone> GroundedOperationTCO<V> for ModOp {
    fn name(&self) -> &str {
        "%"
    }
    ...
    fn execute_step<F: MettaValueFactory<V>>(
        ...
        match state.step {
            ...
            _ => unreachable!("Invalid step {} for ModOp", state.step),
        }
    }
}

/// TCO Minimum operation: (min a b)
pub struct MinOp;

impl<V: MettaValueTrait + Clone> GroundedOperationTCO<V> for MinOp {
    fn name(&self) -> &str {
        "min"
    }
    ...
    fn execute_step<F: MettaValueFactory<V>>(
        ...
        match state.step {
            ...
            _ => unreachable!("Invalid step {} for MinOp", state.step),
        }
    }
}

/// TCO Maximum operation: (max a b)
pub struct MaxOp;

impl<V: MettaValueTrait + Clone> GroundedOperationTCO<V> for MaxOp {
    fn name(&self) -> &str {
        "max"
    }
    ...
    fn execute_step<F: MettaValueFactory<V>>(
        ...
        match state.step {
            ...
            _ => unreachable!("Invalid step {} for MaxOp", state.step),
        }
    }
}
...
pub struct SafeDivOp;

impl<V: MettaValueTrait + Clone> GroundedOperationTCO<V> for SafeDivOp {
    fn name(&self) -> &str {
        "/safe"
    }
    ...
    fn execute_step<F: MettaValueFactory<V>>(
        ...
        match state.step {
            ...
            _ => unreachable!("Invalid step {} for SafeDivOp", state.step),
        }
    }
}

/// TCO Clamp operation: (clamp val min max)
/// Returns min(max, max(val, min)), clamping val to [min, max].
pub struct ClampOp;

impl<V: MettaValueTrait + Clone> GroundedOperationTCO<V> for ClampOp {
    fn name(&self) -> &str {
        "clamp"
    }
    ...
    fn execute_step<F: MettaValueFactory<V>>(
        ...
        match state.step {
            ...
            1 => {
                ...
                if let Some(err) = find_error(val_results) {
                    return GroundedWork::Done(vec![(err.clone(), None)]);
                }
                state.step = 2;
                GroundedWork::EvalArg {
            ...
            2 => {
                ...
                if let Some(err) = find_error(min_results) {
                    return GroundedWork::Done(vec![(err.clone(), None)]);
                }
                state.step = 3;
                GroundedWork::EvalArg {
            ...
            3 => {
                ...
                if let Some(err) = find_error(max_results) {
                    return GroundedWork::Done(vec![(err.clone(), None)]);
                }
            ...
            _ => unreachable!("Invalid step {} for ClampOp", state.step),
        }
    }
}
...
mod tests {
    ...
    fn test_add_op_generic_longs() {
        ...
        assert!(matches!(work, GroundedWork::EvalArg { arg_idx: 0, .. }));
        ...
        let work = op.execute_step(&mut state, &factory);
        assert!(matches!(work, GroundedWork::EvalArg { arg_idx: 1, .. }));
