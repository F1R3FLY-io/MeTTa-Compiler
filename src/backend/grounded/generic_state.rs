//! Generic State types for TCO grounded operations.
//!
//! Provides generic state machine types for tail-call optimized grounded operations
//! that work with any value type implementing `MettaValueTrait`.
//!
//! - `GenericGroundedWork<V>` - Enum describing what work needs to be done
//! - `GenericGroundedState<V>` - State saved between operation steps
//!
//! ## Design
//!
...
/// Generic work returned by grounded operations for TCO.
///
...
///
/// This generic version works with any value type implementing `MettaValueTrait`.
#[derive(Debug, Clone)]
pub enum GenericGroundedWork<V: MettaValueTrait + Clone> {
...
/// Generic state saved between grounded operation steps.
///
/// This struct is passed to `execute_step_generic` and contains all state needed
...
#[derive(Debug, Clone)]
pub struct GenericGroundedState<V: MettaValueTrait + Clone> {
...
///
/// Generic version that works with any value type implementing `MettaValueTrait`.
pub fn find_error_generic<V: MettaValueTrait>(results: &[V]) -> Option<&V> {
    results.iter().find(|v| v.is_error())
}

/// Get a friendly type name for error messages.
///
/// Generic version that uses `MettaValueTrait::friendly_type_name()`.
pub fn friendly_type_name_generic<V: MettaValueTrait>(value: &V) -> &'static str {
...
mod tests {
    ...
    fn test_generic_grounded_state_new() {
        ...
        let state: GenericGroundedState<MettaValue> = GenericGroundedState::new("+".to_string(), args);
    ...
    fn test_generic_grounded_state_set_get_arg() {
        let args = vec![MettaValue::Long(1), MettaValue::Long(2)];
        let mut state: GenericGroundedState<MettaValue> = GenericGroundedState::new("+".to_string(), args);
    ...
    #[test]
    fn test_find_error_generic() {
        let values = vec![
            MettaValue::Long(1),
            MettaValue::Long(2),
        ];
        assert!(find_error_generic(&values).is_none());

        let values_with_error = vec![
            MettaValue::Long(1),
            MettaValue::Error("test error".to_string(), MettaValue::Nil()),
        ];
        let err = find_error_generic(&values_with_error);
        assert!(err.is_some());
