...
impl ConvertState {
    ...
    /// Ensure all caches are valid for the given environment epoch and GC sweep epoch.
    ///
    /// Two invalidation sources:
    /// 1. Environment epoch change → clear all caches (different symbol table)
    /// 2. GC sweep epoch change → flag ground_cache for per-entry lazy validation
    ///    (slab slots may have been freed and reused with different data)
    #[inline]
...
/// `get_slot_epoch()` uses page metadata (not pointer dereferencing), so
...
pub fn with_mork_query_bytes<V: MettaValueTrait, R>(
    ...
    CONVERT_STATE.with(|state| {
        ...
        // fragments from GC-reused slab slots (ABA problem). The literal path
