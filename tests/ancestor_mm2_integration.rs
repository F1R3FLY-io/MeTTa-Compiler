...
fn test_ancestor_mm2_simple() {
    ...
    let query_s = compile("(match &self (generation (S Z) Ann $gp) (generation (S Z) Ann $gp))").unwrap();
...
fn test_ancestor_mm2_child_derivation() {
    ...
    assert!(!results.is_empty(), "Should derive (child Bob Tom) from (parent Tom Bob)");
    assert!(result.converged, "Should converge");
    ...
    assert!(!results2.is_empty(), "Should derive (child Ann Bob) from (parent Bob Ann)");
}
...
fn test_ancestor_mm2_generation_z() {
    ...
    assert!(!results.is_empty(), "Should generate (generation Z Ann Bob)");
    assert!(result.converged, "Should converge");
}
...
fn test_ancestor_mm2_multiple_generations() {
    ...
    println!("Iterations: {}, Converged: {}, Facts added: {}",
        result.iterations, result.converged, result.facts_added);
    ...
    let query_sz = compile("(match &self (generation (S Z) Ann Tom) (generation (S Z) Ann Tom))").unwrap();
    ...
    assert!(!results_sz.is_empty(), "Should have (generation (S Z) Ann Tom)");
    assert!(result.converged, "Should converge");
}
