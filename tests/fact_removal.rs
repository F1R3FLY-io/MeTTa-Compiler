...
//!
//! Migrated to use the arena API (compile_arena, eval_arena, new_arena_env).
...
fn test_remove_exact_fact() {
    ...
    // Verify it exists
    let query_state = compile_arena("(match &self (foo bar) (foo bar))").expect("compile failed");
    let (results, _) = eval_arena(query_state.source()[0], env.clone(), &query_state);
    ...
    // Verify it's gone
    let (results_after, _) = eval_arena(query_state.source()[0], env.clone(), &query_state);
...
fn test_remove_from_multiple_facts() {
    ...
    let query1_state =
        compile_arena("(match &self (parent Alice Bob) (parent Alice Bob))").expect("compile failed");
    ...
    let query3_state =
        compile_arena("(match &self (parent Carol Dave) (parent Carol Dave))").expect("compile failed");
    let (results3, _) = eval_arena(query3_state.source()[0], env.clone(), &query3_state);
    assert_eq!(results3.len(), 1, "Third fact should still exist");

    // Verify middle is gone
    let query2_state =
        compile_arena("(match &self (parent Bob Carol) (parent Bob Carol))").expect("compile failed");
    let (results2, _) = eval_arena(query2_state.source()[0], env.clone(), &query2_state);
...
fn test_operation_remove_via_direct_api() {
    ...
    let query_state =
        compile_arena("(match &self (temp foo) (temp foo))").expect("compile failed");
    let (results, _) = eval_arena(query_state.source()[0], env.clone(), &query_state);
...
fn test_remove_and_readd() {
    ...
    // Verify it exists
    let query_state =
        compile_arena("(match &self (data test) (data test))").expect("compile failed");
    let (results, _) = eval_arena(query_state.source()[0], env.clone(), &query_state);
...
fn test_remove_multiple_identical_facts() {
    ...
    let query_state =
        compile_arena("(match &self (foo bar) (foo bar))").expect("compile failed");
    let (results, _) = eval_arena(query_state.source()[0], env.clone(), &query_state);
    assert_eq!(
        results.len(),
        2,
    ...
    let (results_after_first_remove, _) =
        eval_arena(query_state.source()[0], env.clone(), &query_state);
    assert_eq!(
