...
fn test_remove_exact_fact() {
    ...
    // SAFE: MutexGuard dropped at semicolon. Prevents ABBA deadlock between
    // source mutex and GC_IN_PROGRESS — see MettaState::source() doc comment.
...
fn test_remove_from_multiple_facts() {
    ...
    // SAFE: MutexGuard dropped at semicolon. Prevents ABBA deadlock between
    // source mutex and GC_IN_PROGRESS — see MettaState::source() doc comment.
    let middle_fact = state.source()[1];
    env.remove_from_space(&middle_fact);
