...
//!
//! **Scope**: these tests exercise the T0 (Treewalker) tier via
//! `eval_with_tier(TierSelection::Treewalker, FallbackPolicy::SilentDemote)`.
//! Auto-tier coverage (which currently still hits a separate bytecode-VM
//! leak path for unbounded user-rule self-recursion) is addressed by the
//! parallel auto-tier-OOM workstream and will be added once that bug is
//! cured.
//!
