...
pub fn metta_atom() -> impl Strategy<Value = MettaValue> {
    prop_oneof![
        ...
        .prop_map(|s| MettaValue::Atom(s.to_string())),
    ]
}
...
fn metta_simple_error() -> impl Strategy<Value = MettaValue> {
    ...
    (error_messages, metta_atom())
        .prop_map(|(msg, detail)| MettaValue::Error(msg.to_string(), Arc::new(detail)))
}

fn metta_simple_type() -> impl Strategy<Value = MettaValue> {
    prop_oneof![
        // Basic type atoms
        Just(MettaValue::Type(Arc::new(MettaValue::Atom(
            "Type".to_string()
        )))),
        Just(MettaValue::Type(Arc::new(MettaValue::Atom(
            "ErrorType".to_string()
