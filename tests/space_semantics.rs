...
fn test_owned_space_variable_atom_match_concrete() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Expected 1 match for stored variable atom"
    );
    assert!(
...
fn test_owned_space_variable_atom_different_vars_unification() {
    ...
    assert_eq!(
        results.len(),
        1,
        "Expected 1 match via unification of free stored-side variables"
    );
    assert!(
        matches!(results[0].inner(), MettaValueInner::Atom(ref s) if *s == "found"),
        "Expected 'found', got {:?}",
...
fn test_owned_space_remove_variable_atom() {
    ...
    assert!(
        results.is_empty(),
        "Expected no matches after removing variable atom, got {:?}",
        results
    );
}
...
fn test_owned_space_chain_resolution() {
    ...
    assert!(
        matches!(results[0].inner(), MettaValueInner::Long(42)),
        "Expected 42 from chain resolution, got {:?}",
        results[0]
    );
}
...
fn test_get_atoms_includes_variable_atoms() {
    ...
    assert_eq!(
        results.len(),
        2,
        "Expected 2 atoms from get-atoms, got {}",
        results.len()
    );
}
...
fn test_mixed_ground_and_variable_match() {
    ...
    assert_eq!(
        results.len(),
        2,
        "Expected 2 matches (ground + variable), got {}",
        results.len()
    );
}
...
fn test_variable_only_pattern_matches_all() {
    ...
    assert_eq!(
        results.len(),
        3,
        "Expected 3 matches for wildcard pattern, got {}",
        results.len()
    );
}
