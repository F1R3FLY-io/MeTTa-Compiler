...
(*                                                                         *)
(* QueryGenGuard models an ABSTRACT serve-point guard: every table entry   *)
(* is stamped with the generation at which it was tabled; a consumer       *)
(* reading at a LATER generation rejects the stale entry and recomputes    *)
(* against the current (complete) derivation, restoring the full answer    *)
(* set.                                                                     *)
(*                                                                         *)
...
(*                                                                         *)
(* NOTE (2026-06-18): QueryGenGuard was a CANDIDATE fix that was NOT the    *)
(* route taken. This module is retained as the abstract freeze-CLASS model *)
(* (a partial/empty table entry frozen and served = a dropped subset). The *)
(* IMPLEMENTED fix is PRECISE SEEDING — see SubgoalSeedOverCutIsolation.tla *)
(* (commit 2ecdf677), which models the concrete mechanism: the cross-thread*)
(* active-set seed over-cut a NON-recursive constant to EMPTY, and seeding  *)
(* only genuinely recursive subgoals prevents the over-cut while keeping    *)
(* the genuine-recursion cut. Both guards independently restore            *)
(* completeness; precise seeding does so at the source (no empty partial is *)
(* ever produced for a constant) rather than at the serve point.           *)
(***************************************************************************)
