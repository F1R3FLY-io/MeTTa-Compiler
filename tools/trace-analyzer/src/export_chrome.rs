...
pub fn run(file: &str, output: &str) -> Result<(), String> {
    ...
    if reader.format_version < 2 {
        return Err(
            "The 'export-chrome' subcommand requires format v2 trace files with duration data. \
    ...
    write!(writer, "{{\"traceEvents\":[").map_err(|e| format!("Write error: {e}"))?;
...
fn event_name(kind: &TraceEventKind) -> String {
    match kind {
        ...
        TraceEventKind::ApplicativePreEval { operator, .. } => {
            format!("ApplicativePreEval:{operator}")
        }
        ...
        TraceEventKind::TierDispatch { selected_tier, .. } => {
            format!("TierDispatch:{selected_tier}")
        }
        ...
        TraceEventKind::InferredTypeRegistered { function_name, .. } => {
            format!("InferredType:{function_name}")
        }
        ...
        TraceEventKind::WorkPoolTaskCompleted { task_kind, .. } => {
            format!("WP:Complete:{task_kind}")
        }
        ...
        TraceEventKind::WorkPoolWorkerResumed { worker_id, .. } => {
            format!("WP:Resume[{worker_id}]")
        }
        TraceEventKind::WorkPoolBlockedWorkersDetected { blocked_count, .. } => {
            format!("WP:Blocked:{blocked_count}")
        }
        TraceEventKind::WorkPoolCompensatoryAction { .. } => "WP:Compensate".to_string(),
        TraceEventKind::WorkPoolMonitorTick { .. } => "WP:Tick".to_string(),
        TraceEventKind::WorkPoolWorkerBlocked { worker_id, .. } => {
            format!("WP:WorkerBlocked[{worker_id}]")
        }
        TraceEventKind::WorkPoolWorkerUnblocked { worker_id, .. } => {
            format!("WP:WorkerUnblocked[{worker_id}]")
        }
        TraceEventKind::LetBindingStep { form, .. } => format!("LetBindingStep:{form}"),
        TraceEventKind::ArgumentPreEvalResult { arg_index, .. } => {
            format!("ArgPreEval[{arg_index}]")
        }
        ...
        TraceEventKind::RuleSelected {
            ...
            total_matches,
            ..
        } => format!("RuleSelected:{selected_index}/{total_matches}"),
        TraceEventKind::RuleMatchAttempt {
            call_head, matcher, ..
        } => format!("RuleMatchAttempt:{matcher}:{call_head}"),
        TraceEventKind::RuleLookup { head, .. } => format!("RuleLookup:{head}"),
        TraceEventKind::RuleIndexInsert { head, .. } => {
            format!("RuleIndexInsert:{}", head.as_deref().unwrap_or("*"))
        }
        ...
        TraceEventKind::ParallelDispatch {
            ...
            phase,
            ..
        } => format!("ParallelDispatch({branch_count}):{phase}"),
        TraceEventKind::TrampolineStep {
            work_kind,
            iteration,
            ..
        } => format!("TrampolineStep#{iteration}:{work_kind}"),
        TraceEventKind::ContinuationEnter {
            cont_kind, flow_id, ..
        } => format!("ContEnter#{flow_id}:{cont_kind}"),
        TraceEventKind::ContinuationEmit {
            cont_kind, flow_id, ..
        } => format!("ContEmit#{flow_id}:{cont_kind}"),
        TraceEventKind::ContinuationExitNoResume {
            cont_kind,
            flow_id,
            exit_kind,
        } => format!("ContExit#{flow_id}:{cont_kind}:{exit_kind}"),
        TraceEventKind::BindingsDropped {
            ...
            "BindingsDropped#{flow_id}:{cont_kind}:{}",
            dropped_keys.join(",")
        ),
        TraceEventKind::BindingsExtracted {
            ...
            arity,
            ..
        } => format!("BindingsExtracted:{source}:{head}/{arity}"),
        TraceEventKind::BindingsFreshened { source, epoch, .. } => {
            format!("BindingsFreshened:{source}:epoch={epoch}")
        }
